...
/*   Updated: 2018/06/25 00:16:08 by lfabbro          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
NetIfModule::NetIfModule(int posx, int posy):
	MonitorModule(NETIFMOD_X,NETIFMOD_Y, posx,posy, "net","  Net Interfaces  "){
}
...
void		NetIfModule::_update(void) {
	...
	while (ptr != NULL && i < IFAMAX) {
		if (ptr->ifa_addr)
		{
			...
			/* Link layer interface (MAC address) */
			if ((ptr->ifa_addr)->sa_family == AF_LINK)
			{
				if (ptr->ifa_addr) {
					this->_interface[i].ether = "";
					tmp = (unsigned char *)LLADDR(
